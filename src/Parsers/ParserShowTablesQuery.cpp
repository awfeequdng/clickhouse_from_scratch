...
bool ParserShowTablesQuery::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    if (s_databases.ignore(pos, expected))
    {
        ...
        {
            std::cout << "not implement !exp_elem.parse(pos, query->limit_length, expected" << std::endl;
            return false;
    ...
    else
    {
        ...
        if (s_from.ignore(pos, expected) || s_in.ignore(pos, expected))
        {
            ...
            //     return false;
            std::cout << "not implement s_from.ignore(pos, expected) || s_in.ignore(pos, expected)" << std::endl;
            return false;
        }
        ...
        {
            std::cout << "not implement s_where.ignore(pos, expected))" << std::endl;
            return false;
    ...
    std::cout << "tryGetIdentifierNameInto(database, query->from);" << std::endl;
