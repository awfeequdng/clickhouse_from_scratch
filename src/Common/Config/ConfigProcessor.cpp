        ...
        std::unordered_set<std::string> & contributing_zk_paths)
{
    ...
    if (included_something)
        doIncludesRecursive(config, include_from, node, contributing_zk_paths);
    else
    {
        NodeListPtr children = node->childNodes();
        Node * child = nullptr;
        for (size_t i = 0; (child = children->item(i)); ++i)
            doIncludesRecursive(config, include_from, child, contributing_zk_paths);
    }
}
    ...
    bool * has_zk_includes)
{
    ...
    try
    {
        ...
        if (node)
        {
            ...
            doIncludesRecursive(config, nullptr, node, contributing_zk_paths);
        ...
        doIncludesRecursive(config, include_from, getRootNode(config.get()), contributing_zk_paths);
    }
    catch (Exception & e)
    {
    ...
    if (!contributing_zk_paths.empty())
    {
        ...
        bool fallback_to_preprocessed)
{
    ...
    try
    {
        config_xml = processConfig(&has_zk_includes);
